                              ...
                              const char *listen_addr_str)
{
    ...
    if(flag == HG_TRUE) { /* RPCs already registered */
        ...
        margo_registered_name(client->mid, "put_st_server_rcmc_rpc", &client->put_st_server_rcmc_id,
                              &flag);
        ...
        client->put_st_id = MARGO_REGISTER(client->mid, "put_st_rpc", bulk_gdim_t,
                                        bulk_out_t, NULL);
        client->put_st_server_rcmc_id = MARGO_REGISTER(client->mid, "put_st_server_rcmc_rpc",
                                        bulk_gdim_t, bulk_out_t, NULL);
        client->query_layout_v3_id = MARGO_REGISTER(client->mid, "query_layout_v3_rpc", odsc_gdim_t,
                                          odsc_list_t, NULL);
        client->get_st_id =
                     ...
                     obj_descriptor **odsc_tab)
{
    ...
    hret = margo_create(client->mid, server_addr, client->query_layout_v2_id, &handle);
    if(hret != HG_SUCCESS) {
        fprintf(stderr, "ERROR: %s: margo_create() failed with %d.\n", __func__,
                hret);
        return (0);
...
// dspaces_put_layout for method 1&2
static int put_layout_v1n2(dspaces_client_t client, const char *var_name, unsigned int ver,
...
// dspaces_put_layout for method 1&2
static int put_layout_v3(dspaces_client_t client, const char *var_name, unsigned int ver,
                int elem_size, int ndim, uint64_t *lb, uint64_t *ub, enum ds_layout_type src_layout,
                const void *data)
{
    hg_addr_t server_addr;
    ...
    get_server_address(client, &server_addr);
    /* create handle */
    hret = margo_create(client->mid, server_addr, client->put_st_server_rcmc_id, &handle);
    if(hret != HG_SUCCESS) {
        fprintf(stderr, "ERROR: (%s): margo_create() failed\n", __func__);
        margo_bulk_free(in.handle);
        return dspaces_ERR_MERCURY;
                    ...
                    void *data)
{
    ...
    // st convert for rm2cm/cm2rm st check
    ...
    // odsc roll back for rm2cm/cm2rm st check
    if(odsc_tab[0].src_st == column_major) {
        temp_od->obj_desc = temp_odsc1;
    }
                ...
                void *data, int timeout)
{
    ...
    num_odscs = get_odscs_layout_v2(client, &odsc, timeout, &odsc_tab);
...
static int get_odscs_layout_v3(dspaces_client_t client, obj_descriptor *odsc, int timeout,
                     obj_descriptor **odsc_tab)
{
    struct global_dimension od_gdim;
    int num_odscs;
    ...
    hret = margo_create(client->mid, server_addr, client->query_layout_v3_id, &handle);
    if(hret != HG_SUCCESS) {
        fprintf(stderr, "ERROR: %s: margo_create() failed with %d.\n", __func__,
                hret);
        return (0);
...
static int get_data_st(dspaces_client_t client, int num_odscs,
                    obj_descriptor req_obj, obj_descriptor *odsc_tab,
                    void *data)
{
    bulk_in_t *in;
    ...
    for(int i = 0; i < num_odscs; ++i) {
        ...
        //} else {
            DEBUG_OUT("retrieving object from server storage.\n");
            margo_create(client->mid, server_addr, client->get_st_id, &handle);
        //}
        margo_request req;
        // forward get requests
        margo_iforward(handle, &in[i], &req);
...
//v3:RCMC at server when data is put
static int get_layout_v3(dspaces_client_t client, const char *var_name, unsigned int ver,
                int elem_size, int ndim, uint64_t *lb, uint64_t *ub, enum ds_layout_type dst_layout, 
                void *data, int timeout)
{
    obj_descriptor temp_odsc, odsc;
    ...
    // try my best to get odscs for existing dst_layout data, if cannot hit, fetch odscs in src_layout 
    num_odscs = get_odscs_layout_v3(client, &odsc, timeout, &odsc_tab);
                ...
                void *data, int timeout)
{
    ...
    case 3:
        get_layout_v3(client, var_name, ver, elem_size, ndim, lb, ub, 
                        dst_layout, data, timeout);
        break;
