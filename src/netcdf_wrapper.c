...
int netcdf_open_rd(char* filepath, int* ncid)
{
    ...
    if(ret != NC_NOERR) {
        fprintf(stderr,"%s, line %i (%s): %s", __FILE__,
                __LINE__, __func__, nc_strerror(ret));
    }
    return ret;
}
...
{
    int ret = 0;
    ret = nc_inq_path(ncid, NULL, filepath);
    if(ret != NC_NOERR) {
        fprintf(stderr,"%s, line %i (%s): %s", __FILE__,
                __LINE__, __func__, nc_strerror(ret));
    }
...
// TODO: maybe change nts type to size_t?
int netcdf_inq_var_timestep(int ncid, int varid, int* tidx, int* nts)
{
    ...
    for(int i=0; i<var_ndims; i++) {
        ...
        if(strcmp(dimname, "time") == 0) { //equal
            *tidx = i;
            ret = nc_inq_dimlen(ncid, var_dimids[i], &ntimestep);
            if(ret != NC_NOERR) {
                fprintf(stderr,"%s, line %i (%s): %s", __FILE__,
                        __LINE__, __func__, nc_strerror(ret));
                return ret;
...
int netcdf_inq_var_elemsize(int ncid, int varid, size_t* elemsize)
{
    ...
    ret = nc_inq_type(ncid, var_type, NULL, elemsize);
    if(ret != NC_NOERR) {
        fprintf(stderr,"%s, line %i (%s): %s", __FILE__,
                __LINE__, __func__, nc_strerror(ret));
    }
